...
function doGet(e) { return handleRequest_(e && e.parameter ? e.parameter : {}); }
function doPost(e) {
  try {
...
function handleRequest_(payload) {
  try {
    ...
    if (action === 'setup' || action === 'diagnostics') return jsonResponse_(setupDiagnostics_());
    ...
    if (action === 'saveRecord') { saveRecord_(payload); return jsonResponse_(buildDashboardResponse_(payload.sheet)); }
    if (action === 'cancelRecord') { cancelRecord_(payload); return jsonResponse_(buildDashboardResponse_(payload.sheet)); }
    if (action === 'deleteRecord' || action === 'hardDeleteRecord') { deleteRecord_(payload); return jsonResponse_(buildDashboardResponse_(payload.sheet)); }
...
function saveRecord_(payload) {
  ...
  try {
    ...
    if (rowIndex > 1 && rowIndex <= sheet.getLastRow()) {
      ...
      assertNoDuplicateOrder_(sheet, record.orderNumber, record.id);
    ...
    assertNoDuplicateOrder_(sheet, record.orderNumber, '');
...
function cancelRecord_(payload) {
  ...
  try {
    const sheet = getDataSheet_(payload.sheet); ensureHeaders_(sheet);
    const rowIndex = resolveRowIndex_(sheet, payload);
    const previous = rowToRecord_(sheet.getRange(rowIndex,1,1,CONFIG.HEADERS.length).getValues()[0],rowIndex);
    const record = Object.assign({},previous,{ status:'cancelado', updatedAt:new Date(), responsible:String(payload.responsible || previous.responsavel || '').trim() });
    sheet.getRange(rowIndex,1,1,CONFIG.HEADERS.length).setValues([recordToRow_(record)]);
...
function resolveRowIndex_(sheet, payload) {
  ...
  if (id) {
    const byId = findRowById_(sheet,id);
...
function assertNoDuplicateOrder_(sheet, orderNumber, currentId) {
  const normalized = String(orderNumber || '').trim().toLowerCase();
  if (!normalized || sheet.getLastRow() < 2) return;
  const rows = sheet.getRange(2,1,sheet.getLastRow()-1,CONFIG.HEADERS.length).getValues();
  const duplicate = rows.some(row => String(row[2] || '').trim().toLowerCase() === normalized && String(row[0] || '') !== String(currentId || '') && normalizeStatus_(row[11]) !== 'cancelado');
  if (duplicate) throw new Error('Já existe uma OS ativa com esse número nesta aba.');
}
...
function deleteBalance_(payload) {
  const lock = LockService.getScriptLock(); lock.waitLock(10000);
  try {
    const id = String(payload.id || '').trim();
    if (!id) throw new Error('Aporte não informado.');
    const sheetName = String(payload.sheet || '').trim();
    getDataSheet_(sheetName);
    const sheet = getOrCreateBalanceSheet_();
    const lastRow = sheet.getLastRow();
    if (lastRow < 2) throw new Error('Aporte não encontrado.');
    const rows = sheet.getRange(2,1,lastRow-1,CONFIG.BALANCE_HEADERS.length).getValues();
    const index = rows.findIndex(r => String(r[0]) === id && String(r[2]) === sheetName);
    if (index === -1) throw new Error('Aporte não encontrado.');
    const rowIndex = index + 2;
    const row = rows[index];
    const previous = { id:row[0], date:toIsoString_(row[1]), sheet:row[2], value:Number(row[3]) || 0, note:row[4] || '', responsible:row[5] || '' };
    appendLog_('EXCLUIR_SALDO',previous.id,sheetName,String(payload.responsible || previous.responsible || '').trim(),previous,null);
    sheet.deleteRow(rowIndex);
  } finally { lock.releaseLock(); }
}
...
function getOrCreateBalanceSheet_() {
  ...
  if (!sheet) { sheet = ss.insertSheet(CONFIG.BALANCE_SHEET); sheet.getRange(1,1,1,CONFIG.BALANCE_HEADERS.length).setValues([CONFIG.BALANCE_HEADERS]); sheet.setFrozenRows(1); }
  return sheet;
}

function setupDiagnostics_() {
  const props = PropertiesService.getScriptProperties();
  const spreadsheetId = props.getProperty(CONFIG.SPREADSHEET_ID_PROPERTY);
  if (!spreadsheetId) return { ok:false, checks:{ spreadsheetId:false }, error:'Configure a propriedade SPREADSHEET_ID.' };
  const ss = getSpreadsheet_();
  getOrCreateBalanceSheet_();
  ensureLogSheet_();
...
function ensureLogSheet_() {
  ...
  if (!sheet) { sheet = ss.insertSheet(CONFIG.LOG_SHEET); sheet.getRange(1,1,1,CONFIG.LOG_HEADERS.length).setValues([CONFIG.LOG_HEADERS]); sheet.setFrozenRows(1); }
  return sheet;
}
...
function recordToRow_(r) { return [r.id,r.createdAt,r.orderNumber,r.plate,r.model,r.chassis,r.description,r.partCode,r.partsValue,r.laborValue,r.total,r.status,r.notes,r.responsible,r.updatedAt]; }
function rowToRecord_(row,rowIndex) { return { id:row[0], createdAt:toIsoString_(row[1]), os:row[2], placa:row[3], modelo:row[4], chassi:row[5], descricao:row[6], codigoPeca:row[7], pecas:Number(row[8]) || 0, maoObra:Number(row[9]) || 0, total:Number(row[10]) || 0, status:normalizeStatus_(row[11]), observacoes:row[12] || '', responsavel:row[13] || '', updatedAt:toIsoString_(row[14]), rowIndex:rowIndex }; }
function normalizeStatus_(value) { const s = String(value || '').trim().toLowerCase(); if (s === 'ok' || s === 'finalizado' || s === 'finalizada') return 'ok'; if (s === 'cancelado') return 'cancelado'; return 'pendente'; }
function parseMoney_(value) { if (typeof value === 'number') return roundCurrency_(value); const normalized = String(value ?? '').trim().replace(/\s/g,'').replace(/R\$/gi,'').replace(/\./g,'').replace(',','.'); if (!normalized) return 0; const n = Number(normalized); if (!Number.isFinite(n)) throw new Error('Valor financeiro inválido.'); return roundCurrency_(n); }
